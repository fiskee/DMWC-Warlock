...
function Warlock.Rotation()
    ...
    if Target and Target.ValidEnemy and Target.Distance < 40 then
        ...
        if not Player.Casting then
            ...
            if Setting("Fear Solo Farming") and not Player.Moving and Target.TTD > 3 and #DMW.Friends.Units < 2 and Spell.ShadowBolt:LastCast() and Debuff.Fear:Count() == 0 and (not Spell.Fear:LastCast() or (DMW.Player.LastCast[1].SuccessTime and (DMW.Time - DMW.Player.LastCast[1].SuccessTime) > 0.7)) and Spell.Fear:Cast(Target) then 
                return true
            end
